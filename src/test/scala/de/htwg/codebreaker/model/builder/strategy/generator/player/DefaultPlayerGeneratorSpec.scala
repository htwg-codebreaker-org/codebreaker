...
class DefaultPlayerGeneratorSpec extends AnyWordSpec with Matchers:

  "DefaultPlayerGenerator" should {

    "generate correct number of players" in {
      val map = WorldMap.defaultMap
      val players = DefaultPlayerGenerator.generatePlayers(3, map)
    ...
    "generate players with sequential IDs starting from 1" in {
      val map = WorldMap.defaultMap
      val players = DefaultPlayerGenerator.generatePlayers(4, map)
    ...
    "place players on land tiles only" in {
      val map = WorldMap.defaultMap
      val players = DefaultPlayerGenerator.generatePlayers(5, map)
    ...
    "avoid specified tiles when placing players" in {
      ...
      val players = DefaultPlayerGenerator.generatePlayers(3, map, List(avoidTile))
    ...
    "generate players with initial laptop hardware" in {
      val map = WorldMap.defaultMap
      val players = DefaultPlayerGenerator.generatePlayers(2, map)

      players.foreach { player =>
        player.laptop.hardware.cpu shouldBe 20
        player.laptop.hardware.ram shouldBe 20
        player.laptop.hardware.code shouldBe 20
        player.laptop.hardware.kerne shouldBe 1
        player.laptop.hardware.networkRange shouldBe 0
      }
    }

    "generate players with initial cybersecurity level" in {
      val map = WorldMap.defaultMap
      val players = DefaultPlayerGenerator.generatePlayers(2, map)

      players.foreach { player =>
        player.laptop.cybersecurity shouldBe 5
      }
    }
    ...
    "generate players with zero initial XP" in {
      val map = WorldMap.defaultMap
      val players = DefaultPlayerGenerator.generatePlayers(2, map)
    ...
    "generate players with initial movement points" in {
      val map = WorldMap.defaultMap
      val players = DefaultPlayerGenerator.generatePlayers(2, map)
    ...
    "generate single player when count is 1" in {
      val map = WorldMap.defaultMap
      val players = DefaultPlayerGenerator.generatePlayers(1, map)
    ...
    "handle generation with multiple avoided tiles" in {
      ...
      val players = DefaultPlayerGenerator.generatePlayers(3, map, avoidTiles)
