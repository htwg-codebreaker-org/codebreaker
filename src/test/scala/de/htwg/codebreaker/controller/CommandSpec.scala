...
class DummyCommand extends Command {
  override def doStep(game: Game) = Success(game.copy(state = game.state.copy(round = game.state.round + 1)))
  override def undoStep(game: Game) = Success(game.copy(state = game.state.copy(round = game.state.round - 1)))
}
