...
/** Repräsentation der Weltkarte als Raster von Tiles.
  ...
  *   Höhe in Tiles
  * @param tiles
  *   Flache Liste aller Tiles mit Position und Kontinent
  */
case class WorldMap(width: Int, height: Int, tiles: Vector[Tile]):

  /** Erzeugt eine 2D‑Matrix von MapObject, die für jede Position angibt, ob dort Spieler, Server,
    * beides oder nichts steht.
    *
    * Wirft IllegalArgumentException, falls auf ungültige Koordinaten zugegriffen wird
    * (tileAt(x,y).get).
    */
...
object WorldMap:
  ...
  /** Ordnet jeder (x,y)‑Koordinate einen Kontinent zu. Bereiche sind für 80x40 Tiles und
    * orientieren sich an deinen Markierungen. Ergänze weitere Bereiche nach Bedarf!
    */
  ...
  def printContinentMap(map: WorldMap): Unit =
    for y <- 0 until map.height do
      val row = for x <- 0 until map.width yield map.continentAt(x, y).map(_.short).getOrElse("--")
