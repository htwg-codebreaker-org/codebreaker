...
/** ServerGenerator kümmert sich um die Erzeugung von Servern auf der Karte:
  *   - Fixed Servers: Aus einer vordefinierten Liste von Blueprints entstehen immer die gleichen
  *     Kern-Server (z.B. "Pentagon").
  *   - Side Servers: Zufällig verteilte Neben-Server pro Kontinent, mit Mindestabstand zueinander
  *     und zu den Fixed Servers.
  */
object ServerGenerator:
  ...
  /** Manhattan‐Abstand zwischen zwei Tiles: distance((x1,y1), (x2,y2)) = |x1-x2| + |y1-y2| Wird
    * genutzt, um Mindestabstände zwischen Servern sicherzustellen.
    */
  private[model] def distance(a: Tile, b: Tile): Int =
    math.abs(a.x - b.x) + math.abs(a.y - b.y)

  /** Wählt bis zu `count` verschiedene Tiles aus `tiles` aus, so dass jeder neu hinzugefügte Tile
    * mindestens `minDistance` (Manhattan‐Abstand) zu allen bereits gewählten und zusätzlich zu den
    * in `avoidTiles` übergebenen Tiles steht.
    *
    * Ablauf:
    *   1. `tiles` wird zufällig durchmischt. 2. Jeder Tile aus dem Shuffle wird nacheinander
    *      geprüft:
    *      - Haben wir bereits `count` Tiles? Dann Abbruch.
    *      - Steht `next` mindestens `minDistance` von allen bisher gewählten (`picked`) und
    *        `avoidTiles` entfernt? Dann hinzufügen.
  ...
  private[model] def pickNonCloseTiles(
    ...
    shuffled.foldLeft(Vector.empty[Tile]) { (picked, next) =>
      if picked.size >= count then picked
  ...
  /** Generiert pro Kontinent eine zufällige Anzahl (3–6) von Neben‐Servern.
    *
    * @param continent
    *   Kontinent, auf dem die Neben‐Server entstehen.
    * @param map
    *   die Weltkarte, um alle Tiles dieses Kontinents abzufragen.
    * @param existingServers
    *   bereits platzierte Server (werden als `avoidTiles` verwendet, damit keine Side‐Server drauf
    *   stehen).
    * @param minDistance
    *   minimale Manhattan‐Distanz zwischen allen Side‐Servern und zu den `existingServers`
    *   (Standard: 2).
    * @return
    *   Liste neu erzeugter Server mit dynamischem Namen, Zufalls‐Difficulty und -Rewards.
    */
  ...
  /** Erzeugt alle Fixed‐Server gemäß den `fixedBlueprints`. Jeder Blueprint gibt einen Namen, eine
    * feste Position und Werte‐Spannen vor. Die tatsächlichen Difficulty‐ und Reward‐Werte werden
    * per `rngIn` gezogen.
    */
  ...
  /** Ziehe eine Zufallszahl im gegebenen inklusiven Bereich (min, max). Wenn min == max, gib direkt
    * den einzigen Wert zurück.
    */
