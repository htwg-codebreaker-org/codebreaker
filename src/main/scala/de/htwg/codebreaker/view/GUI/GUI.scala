...
import de.htwg.codebreaker.controller.{
  Command,
  ControllerInterface,
  HackServerCommand,
  MovePlayerCommand,
...
/** Graphical User Interface component using ScalaFX. Depends only on ControllerInterface, not on
  * the concrete Controller implementation. Implements Observer to receive updates when the game
  * state changes.
  *
  * @param controller
  *   The game controller, injected by Guice
  */
class GUI @Inject() (val controller: ControllerInterface) extends JFXApp3 with Observer:
  ...
  def serverIconFile(serverType: ServerType): String = serverType match {
    case ServerType.Side     =>
      "src/main/scala/de/htwg/codebreaker/assets/graphics/server/icons/side_server.png"
    case ServerType.Firm     =>
      "src/main/scala/de/htwg/codebreaker/assets/graphics/server/icons/firm_server.png"
    case ServerType.Cloud    =>
      "src/main/scala/de/htwg/codebreaker/assets/graphics/server/icons/cloud_server.png"
    case ServerType.Bank     =>
      "src/main/scala/de/htwg/codebreaker/assets/graphics/server/icons/bank_server.png"
    case ServerType.Military =>
      "src/main/scala/de/htwg/codebreaker/assets/graphics/server/icons/military_server.png"
    case ServerType.GKS      =>
      "src/main/scala/de/htwg/codebreaker/assets/graphics/server/icons/gks_server.png"
    case ServerType.Private  =>
      "src/main/scala/de/htwg/codebreaker/assets/graphics/server/icons/player_base.png"
  }
  ...
  def showWorldMap(): Unit = {
    ...
    val bgImage = new Image(
      new FileInputStream("src/main/scala/de/htwg/codebreaker/assets/graphics/landscape.png")
    )
    ...
    for (server <- servers) {
      ...
      iconView.layoutX <== stage.width.divide(map.width).multiply(server.tile.x) + stage.width
        .divide(map.width) * 0.2
      iconView.layoutY <== stage.height.divide(map.height).multiply(server.tile.y) + stage.height
        .divide(map.height) * 0.2
    ...
    for ((player, index) <- players.zipWithIndex) {
      val playerIcon  = new scalafx.scene.shape.Circle {
        ...
        centerX <== stage.width.divide(map.width).multiply(player.tile.x) + stage.width.divide(
          map.width
        ) * 0.5
        centerY <== stage.height.divide(map.height).multiply(player.tile.y) + stage.height.divide(
          map.height
        ) * 0.5
      ...
      val playerLabel = new Label(s"P$index") {
        layoutX <== stage.width.divide(map.width).multiply(player.tile.x) + stage.width.divide(
          map.width
        ) * 0.35
        layoutY <== stage.height.divide(map.height).multiply(player.tile.y) + stage.height.divide(
          map.height
        ) * 0.35
    ...
    val playerInfoBox      = new VBox {
      style =
        "-fx-background-color: #222; -fx-padding: 10; -fx-border-radius: 8; -fx-background-radius: 8; -fx-border-color: #888; -fx-border-width: 2;"
      spacing = 6
      children = Seq(
        new Label(s"Spieler: ${player.name}")                                               {
          style = "-fx-text-fill: white; -fx-font-weight: bold;"
        },
        new Label(s"CPU: ${player.cpu}   RAM: ${player.ram}   Code: ${player.code}")        {
          style = "-fx-text-fill: #ccc;"
        },
        new Label(s"Bewegungspunkte: ${player.movementPoints}/${player.maxMovementPoints}") {
          style = "-fx-text-fill: #4db8ff; -fx-font-weight: bold;"
        },
    ...
    val topBar = new scalafx.scene.layout.HBox {
      style =
        "-fx-background-color: #222; -fx-padding: 10; -fx-spacing: 20; -fx-alignment: center-left;"
      children = Seq(
        new Label(s"Runde: ${controller.getState.round}") {
          style = "-fx-text-fill: #fff; -fx-font-size: 16; -fx-font-weight: bold;"
        },
        new Label(
          s"Am Zug: ${controller.getPlayers(controller.getState.currentPlayerIndex.getOrElse(0)).name}"
        )                                                 {
