...
/** Text-based User Interface component. Depends only on ControllerInterface, not on the concrete
  * Controller implementation. Implements Observer to receive updates when the game state changes.
  *
  * @param controller
  ...
  def processInputLine(input: String): Unit =
    input.trim match
      ...
      case s if s.startsWith("hack ") =>
        ...
        else println("Syntax: hack <Servername>")
  ...
  def show(): Unit =
    ...
    players.zipWithIndex.foreach {
      case (p, i) =>
        ...
        println(
          s"CPU: ${p.cpu} | RAM: ${p.ram} | Code: ${p.code} | Bewegung: ${p.movementPoints}/${p.maxMovementPoints}"
        )
